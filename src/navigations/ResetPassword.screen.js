      ...
      <div className="flex flex-col min-h-screen bg-white p-4">
        <div className="flex-1 flex flex-col items-center justify-center">
          <div className="bg-red-100 text-red-700 p-4 rounded-md mb-4 max-w-md w-full">
            <p>Missing email information. Please go back to the forgot password page.</p>
          </div>
          <Link
            to="/forgot-password"
    ...
    <div className="flex flex-col min-h-screen bg-white">
      <div className="sticky top-0 z-10 bg-white shadow-sm px-4 py-3">
        <div className="flex items-center">
          ...
          <h1 className="text-lg font-semibold text-center flex-1">Reset Password</h1>
        ...
        <div className="max-w-md mx-auto">
          ...
          <form onSubmit={handleResetPassword} className="space-y-6">
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full p-3 pr-10 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-accent"
                  value={newPassword}
              ...
              {newPassword && (
                <div className="mt-2">
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <span className="text-xs font-medium" style={{ color: passwordStrength >= 3 ? '#22c55e' : '#ef4444' }}>
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-y-1 gap-x-2">
                    <div className="flex items-center text-xs">
                      <span className={`mr-1 ${/[A-Z]/.test(newPassword) ? 'text-green-500' : 'text-gray-400'}`}>
                    ...
                    <div className="flex items-center text-xs">
                      <span className={`mr-1 ${/[0-9]/.test(newPassword) ? 'text-green-500' : 'text-gray-400'}`}>
                        {/[0-9]/.test(newPassword) ? '✓' : '○'}
                      </span>
                      <span className="text-gray-600">Number</span>
                    </div>
                    <div className="flex items-center text-xs">
                      <span className={`mr-1 ${/[\W_]/.test(newPassword) ? 'text-green-500' : 'text-gray-400'}`}>
                        {/[\W_]/.test(newPassword) ? '✓' : '○'}
                      </span>
            ...
            <div>
              ...
              {confirmPassword && (
                <div className="mt-1 flex items-center">
                  <span className={`mr-1 ${confirmPassword === newPassword ? 'text-green-500' : 'text-red-500'}`}>
                    {confirmPassword === newPassword ? '✓' : '✗'}
                  </span>
                  <span className={`text-xs ${confirmPassword === newPassword ? 'text-green-600' : 'text-red-600'}`}>
            ...
            <button
              ...
              disabled={passwordStrength < 4 || confirmPassword !== newPassword || loading}
            >
              {loading ? (
